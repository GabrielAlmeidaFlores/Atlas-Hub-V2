...
| Superfície | Inclui |
|---|---|
| **Portal da Incorporadora** | Cadastro, perfil, wizard 5 etapas + editar, equipe, viabilidade simplificada, barra de progresso, upload/download S3, acompanhamento de status, notificações (incl. cartão habilitado / liberação confirmada ou recusada), link da oferta quando `OFERTA_CRIADA`, **cronograma da obra** (etapas + orçado; gastos após `APROVADO`/`OFERTA_CRIADA`) |
...
| **Financeiro (Admin, pós-sucesso)** | Conta tesouraria Atlas + workspace por projeto/SPE (mesmo CNPJ Atlas), saldo, extrato, Pix (solicitação + dupla aprovação de dois `ADMIN_MASTER`), split, extrato público para integração, conciliação via webhook, trilha de auditoria. **Cartão (preparação):** pedido único da SPE; limite recalculado no mesmo pedido quando o cronograma muda; liberação só da etapa em andamento (incorporadora solicita, admin confirma). Emissão, CDI, cashback e conta Stark no CNPJ da SPE **não** entram. Comissão Atlas é % da captação, não do cartão. O dinheiro da oferta **não** passa por aqui durante a captação. |
...
- Preparação do cartão da obra: pedido único (`SOLICITADO`); limite = orçado da etapa, atualizado no mesmo pedido se o cronograma mudar; teto vigente = só a etapa em andamento, após pedido da incorporadora e confirmação do admin. Sem emitir cartão, aplicar CDI, creditar cashback ou abrir conta no CNPJ da SPE. Comissão Atlas (8% da captação) fica fora do cartão.
