...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  const log = createLogger('incorporadoraDocumentoPresign');
  try {
    const userId = getUserId(event);
    requirePerfil(event, 'INCORPORADORA');
    const body = validate(presignSchema, JSON.parse(event.body ?? '{}'));
    const key = `incorporadoras/${userId}/empresa/${Date.now()}-${body.fileName}`;
    const result = await generatePresignedUrl(key, body.mimeType);
    log.info('Pre-signed URL generated', { userId, key });
