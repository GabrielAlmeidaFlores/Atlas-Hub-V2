...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  const log = createLogger('cronogramaObter');
  try {
    ...
    log.info('Cronograma loaded', { projetoId, etapas: etapas.length, lancamentos: lancamentos.length });
    return ok(event, cronogramaPayload(projeto, etapas, lancamentos, 'owner'));
  } catch (err) {
