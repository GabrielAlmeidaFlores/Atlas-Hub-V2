...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  const log = createLogger('adminCuradoriaListar');
  try {
    const userId = getUserId(event);
    requireAdmin(event);
    const cursor = (event.queryStringParameters?.['cursor'] ?? null) as string | null;
    const { items, cursor: nextCursor } = await listProjetosByStatus(
      ['SUBMETIDO', 'EM_ANALISE', 'AJUSTE_SOLICITADO'],
      cursor,
    );
    log.info('Curation queue listed', { userId, count: items.length });
